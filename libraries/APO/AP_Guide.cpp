...
void AP_Guide::setCurrentIndex(uint8_t val) {
    ...
    _board->getGcs()->sendMessage(MAVLINK_MSG_ID_WAYPOINT_CURRENT);
    updateCommand();
}
...
void MavlinkGuide::updateCommand() {
    ...
    if (_command.getCommand() == MAV_CMD_NAV_WAYPOINT) {
        ...
        _board->getDebug()->printf_P(PSTR("unhandled command"));
        _board->getGcs()->sendText(SEVERITY_HIGH,PSTR("unhandled command"));
        nextCommand();
        return;
    }
...
void MavlinkGuide::handleCommand() {
    ...
    if (
        ...
        if ( (getDistanceToNextWaypoint() < _command.getRadius()) | (getDistanceToNextWaypoint() < 1e-5) ) {
            _board->getGcs()->sendText(SEVERITY_LOW,PSTR("waypoint reached (distance)"));
            _board->getDebug()->printf_P(PSTR("waypoint reached (distance)"));
            nextCommand();
            return;
        }
        ...
        if (alongTrack > segmentLength) {
            _board->getGcs()->sendText(SEVERITY_LOW,PSTR("waypoint reached (along track)"));
            _board->getDebug()->printf_P(PSTR("waypoint reached (along track) segmentLength: %f\t alongTrack: %f\n"),segmentLength,alongTrack);
            nextCommand();
            return;
        }
    ...
    // if in unhandled mode, then return
        ...
        _board->getDebug()->printf_P(PSTR("unhandled guide mode"));
        _board->getGcs()->sendText(SEVERITY_HIGH,PSTR("unhandled guide mode"));
        _mode = MAV_NAV_RETURNING;
    }
